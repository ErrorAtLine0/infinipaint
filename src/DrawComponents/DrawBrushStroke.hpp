...
class DrawBrushStroke : public DrawComponent {
    ...
    private:
        ...
        void vertices_to_draw_data(const std::vector<SkPoint>& vertexData);
        std::vector<size_t> get_wedge_indices(const std::vector<DrawBrushStrokePoint>& points);
        std::vector<DrawBrushStrokePoint> smooth_points(size_t beginIndex, size_t endIndex);
        std::vector<DrawBrushStrokePoint> smooth_points_avg(size_t beginIndex, size_t endIndex);
#endif
};
