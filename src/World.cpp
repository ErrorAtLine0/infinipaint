...
void World::input_android_text_box_input_callback(const CustomEvents::AndroidTextBoxInputEvent& textboxInput) {
    if(!clientStillConnecting)
        drawProg.input_android_text_box_input_callback(textboxInput);
}
...
void World::save_to_file(const std::filesystem::path& filePathToSaveAt, bool saveThumbnailIfAvailable) {
    try {
        ...
        set_name(filePath.stem().string());
            ...
            else if(saveThumbnail && saveThumbnailIfAvailable) {
