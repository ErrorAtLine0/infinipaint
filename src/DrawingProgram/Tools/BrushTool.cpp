...
void BrushTool::gui_pressure_options() {
    ...
    checkbox_boolean_field(gui, "local correction", "Local wobble correction (experimental)", &main.conf.tabletOptions.penFilter.enabled);
    text_label(gui, "Correction is independent of pressure mode; next stroke.");
    if (main.conf.tabletOptions.penFilter.enabled) {
        slider_scalar_field(gui, "local radius", "Radius (DIP)", &main.conf.tabletOptions.penFilter.radius, 4.0, 20.0, {.decimalPrecision = 1});
        slider_scalar_field(gui, "local window", "Revision (seconds)", &main.conf.tabletOptions.penFilter.window, 0.040, 0.200, {.decimalPrecision = 3});
        slider_scalar_field(gui, "local cap", "Max correction (DIP)", &main.conf.tabletOptions.penFilter.cap, 0.0, 6.0, {.decimalPrecision = 1});
    }
