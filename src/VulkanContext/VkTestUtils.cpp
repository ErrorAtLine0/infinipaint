...
#include <SDL3/SDL_vulkan.h> // Looking for instance required extensions for SDL
...
static bool init_instance_extensions_and_layers(PFN_vkGetInstanceProcAddr getInstProc,
    ...
    // via Vulkan implementation and implicitly enabled layers
    {
        ...
        // Check if SDL extension requirements are met
        uint32_t sdlRequiredExtensionCount = 0;
        char const * const * sdlRequiredExtensions = SDL_Vulkan_GetInstanceExtensions(&sdlRequiredExtensionCount);
        ...
        for (uint32_t i = 0; i < extensionCount; ++i) {
            if (should_include_extension(extensions[i].extensionName)) {
                ...
                for(uint32_t j = 0; j < sdlRequiredExtensionCount; j++) {
                    if(strcmp(extensions[i].extensionName, sdlRequiredExtensions[j]) == 0) {
                        instanceExtensions->push_back() = extensions[i];
                        break;
                    }
                }
