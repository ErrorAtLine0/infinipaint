...
CollabTextBox::ShapeResult CollabTextBox::shape(const char* utf8Text,
                                                size_t textByteLen,
                                                const SkFont& font,
...
//#endi
    ...
    if (textByteLen) {
        ...
        //if (!bidi) {
        //    throw std::runtime_error("[CollabTextBox] SkShaper unicode or harfbuzz not available");
    ...
    result.wordBreaks = get_utf8_word_boundaries(utf8Text, textByteLen, locale);
    return result;
}

std::vector<bool> CollabTextBox::get_utf8_word_boundaries(const char* begin, std::size_t byteCount, const char* locale) {
    auto unicode = get_unicode_2();
    if (nullptr == unicode) {
        return {};
    }
    ...
    for (auto& pos : positions) {
        if(pos < result.size()) // Sometimes the position to cut at is out of bounds
            result[pos] = true;
    }
    return result;
}
